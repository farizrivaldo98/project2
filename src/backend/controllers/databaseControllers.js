...
module.exports = {
  ...
  getPowerData: async (request, response) => {
    ...
    let queryData =
      ...
      "'  ORDER BY    p1.date;";
    let queryData2 =
      "SELECT    (@counter := @counter + 1) AS x,    label,    yFROM (    SELECT        p1.date AS label,        p1.id AS x,        p2.`LVMDP1` - p1.`LVMDP1` AS y    FROM        parammachine_saka.power_data p1    JOIN        parammachine_saka.power_data p2 ON p2.date = (            SELECT MIN(date)            FROM parammachine_saka.power_data            WHERE date > p1.date        )    UNION ALL    SELECT        DATE_FORMAT(FROM_UNIXTIME(p1.`time@timestamp`), '%Y-%m-%d') AS label,        p1.data_index AS x,        p2.`data_format_0` - p1.`data_format_0` AS y    FROM        parammachine_saka.`cMT-Gedung-UTY_LVMDP1_data` p1    JOIN        parammachine_saka.`cMT-Gedung-UTY_LVMDP1_data` p2        ON DATE_FORMAT(FROM_UNIXTIME(p2.`time@timestamp`), '%Y-%m-%d') = (            SELECT MIN(DATE_FORMAT(FROM_UNIXTIME(`time@timestamp`), '%Y-%m-%d'))            FROM parammachine_saka.`cMT-Gedung-UTY_LVMDP1_data`            WHERE DATE_FORMAT(FROM_UNIXTIME(`time@timestamp`), '%Y-%m-%d') > DATE_FORMAT(FROM_UNIXTIME(p1.`time@timestamp`), '%Y-%m-%d')        )) AS subqueryCROSS JOIN (SELECT @counter := 0) AS counter_initWHERE    label >= '2023-01-21'    AND label <= '2023-07-03'ORDER BY    x;";
